...
def generate_multi_sheet_excel(all_results: list[dict], output_path: str) -> str:
    """
    生成多 Sheet Excel（每个项目一个 Sheet）

    Args:
        all_results: [{df, project_name, plate, total_supply, total_trans, project_total,
                       month_labels, total_avg_price, monthly_total_prices,
                       open_date, plot_ratio, competitor}, ...]
        output_path: 输出路径
    """
    from openpyxl import Workbook
    ...
    for result in all_results:
        sheet_name = result['project_name'][:31]  # Excel sheet 名最长 31 字符
        ws = wb.create_sheet(title=sheet_name)

        df = result['df']
        month_labels = result['month_labels']
        ...
        # 调用单 Sheet 生成的核心逻辑（内联避免循环导入）
        _write_single_sheet(
            ws, df, total_supply, total_trans, project_total,
            month_labels, project_name, plate, competitor,
            open_date, plot_ratio, total_avg_price, monthly_total_prices
        )
...
def _write_single_sheet(ws, df, total_supply, total_trans, project_total,
                        month_labels, project_name, plate, competitor,
                        open_date, plot_ratio, total_avg_price, monthly_total_prices):
    """写入单个 Sheet（与 generate_excel 逻辑一致）"""
    base_cols = ['板块', '竞品', '业态', '编码', '户型', '户型面积', '建面分段',
                 '整盘套数', '户配', '供应套数', '成交套数', '成交均价',
                 '整盘去化率', '已供去化率', '已供去化占比']
    month_cols = list(month_labels)
